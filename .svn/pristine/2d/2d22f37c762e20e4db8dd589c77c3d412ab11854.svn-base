...
Page({
  ...
  SettlementShopCart: function () {
    var e = this,
      ...
      itemInfo = this.data.StockProducts[0].CartItemInfo,
      contentObj = {},
      itemObj = [],
      type = '标准定制',
      itemObj = itemInfo.forEach((v, i) => {
        if (v.selected === true) {
          // 转换数据格式
          if (typeof v.ProductRemark == 'string' && typeof v.SkuContent == 'string') {
            v.ProductRemark = JSON.parse(v.ProductRemark)
            v.SkuContent.split(';').forEach(n => {
              var key = n.split(':')
              ...
              key[0] ? contentObj[key[0]] = key[1] : delete key[0]
            })
            v.specification = contentObj
          }

          // 判断是否选中的商品
          if (v.selected) {
            // 将选中的商品信息存储
            e.data.formDataNormal.push({
              needStr: v.ProductRemark.text ? v.ProductRemark.text : v.Name,
              materialName: v.MaterialName,
              attachment: v.ProductRemark.img ? v.ProductRemark.img : v.ThumbnailUrl100,
              quantity: v.Quantity,
    ...
    if (e.data.isEdite) {
      ...
      t.getOpenId(function (n) {
        // 遍历选中的商品发请求立即询价
        e.data.formDataNormal.forEach((v, i) => {
          wx.request({
            url: 'https://spapi.zhuanyegou.com/api/values?action=EnterpriseCustomization_Insert',
            method: 'POST',
            data: {
              type: type,
              details: JSON.stringify(e.data.formDataNormal[i]),
